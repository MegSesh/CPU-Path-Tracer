...
//make Camera object camera pointer
void Scene::SetCamera(const Camera &c)  //change signature of this to tack on thin lens camera's 2 vars
{
    camera = Camera(c);
    camera.create();
...
void Scene::SetProjectiveCamera(const Camera &c, const Float lensRadius, const Float focalDistance)
{
    camera = ProjectiveCamera(Camera(c), lensRadius, focalDistance);
    camera.create();
    film.SetDimensions(c.width, c.height);
}
...
void Scene::CreateTestScene()
{
    ...
    camera = Camera(400, 400, Point3f(5, 8, -5), Point3f(0,0,0), Vector3f(0,1,0), 0.5, 1.0);
