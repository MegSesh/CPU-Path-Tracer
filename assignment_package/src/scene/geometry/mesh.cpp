...
Point3f Triangle::sampleSurface(std::shared_ptr<Sampler> sampler, Vector3f* t, Vector3f* b, Normal3f* n) const
{
    return Point3f(0.0f);
}
...
Bounds3f Triangle::WorldBound() const
{
    ...
    Point3f p1 = points[1];
    Point3f p2 = points[2];
//    Bounds3f objBounds = Union(Bounds3f(p0, p1), p2);

    //find min and max of 3 points
    Point3f min(std::min(p0.x, std::min(p1.x, p2.x)),
                std::min(p0.y, std::min(p1.y, p2.y)),
                std::min(p0.z, std::min(p1.z, p2.z))
                );

    Point3f max(std::max(p0.x, std::max(p1.x, p2.x)),
...
void Triangle::ComputeTriangleTBN(const Point3f &P, Normal3f *nor, Vector3f *tan, Vector3f *bit, const Point2f &uv) const
{
    *nor = GetNormal(P);
    ...
    //this doesn't incorporate UV's, but it takes care of the black triangles that appear in naive integrator rendering
    //makes full lighting have fireflies though.
//    CoordinateSystem(P, tan, bit);
}
