...
Point3f Sphere::sampleSurface(std::shared_ptr<Sampler> sampler, Vector3f *t, Vector3f *b, Normal3f *n) const
{
    Point2f pt = sampler->Get2D();
    ComputeTBN(glm::vec3(pt, 0.0), n, t, b);
    Point3f origin = glm::vec3(transform.T() * glm::vec4(WarpFunctions::squareToSphereUniform(pt), 1.0f));
    return origin;
}
